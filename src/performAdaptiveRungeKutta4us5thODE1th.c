...
typedef double(*function)(const double tTemp, const double xTemp);
// // k1 = f(tn, yn)
// int calculateCoeff1(const double t, const double x, const double dt, function y, 
//                     double *result)
// {
//     y(t, x, result);
//     return 0;
// };
// // k2 = f(tn + c2 h, yn + (a21 k1) h)
// int calculateCoeff2(const double t, const double x, const double dt, function y,      
//                     const double coeffPrev, const double c2, const double a21,
//                     double *result)
// {
//     function(t + c2 * dt, x + a21 * coeffPrev, result);
//     *result *= dt;
//     return 0;
// };
...
int calculateNext(const double tPrev, const double xPrev, const double dt,
                  function f, double *xNext, double *xsNext)
{
    ...
    coeff1 = f(tPrev, xPrev);
    coeff2 = f(tPrev+c2*dt, xPrev+(a21*coeff1)*dt);
    coeff3 = f(tPrev+c3*dt, xPrev+(a31*coeff1 + a32*coeff2)*dt);
    coeff4 = f(tPrev+c4*dt, xPrev+(a41*coeff1 + a42*coeff2 + a43*coeff3)*dt);
    coeff5 = f(tPrev+c5*dt, xPrev+(a51*coeff1 + a52*coeff2 + a53*coeff3 + a54*coeff4)*dt);
    coeff6 = f(tPrev+c6*dt, xPrev+(a61*coeff1 + a62*coeff2 + a63*coeff3 + a64*coeff4 + a65*coeff5)*dt);
